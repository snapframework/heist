...
module Heist.Compiled.Experimental where
...
runChildren2 :: Monad n => HeistT n IO (RuntimeSplice n Builder)
runChildren2 = do
    res <- runChildren
    return $! codeGen $! consolidate res

mapCompiledSplices :: Monad n => (a -> n Builder) -> n [a] -> Splice n
mapCompiledSplices f getList = return $ yieldRuntime $ do
    items <- lift getList
    outputs <- forM items (lift . f)
...
runChildrenWith' :: Monad n
                 ...
                 -> n a
                 -> Splice n
runChildrenWith' f splices action = do
    let fieldSplice g = return $ yieldRuntime $ liftM (f . g) $ lift action
    let splices' = map (second fieldSplice) splices
    withLocalSplices splices' [] runChildren

runChildrenWith :: Monad n => [(Text, a -> Builder)] -> n a -> Splice n
runChildrenWith = runChildrenWith' id

runChildrenWithText :: Monad n => [(Text, a -> Text)] -> n a -> Splice n
runChildrenWithText = runChildrenWith' (fromByteString . T.encodeUtf8)

runChildrenWithNodes :: Monad n => [(Text, a -> [X.Node])] -> n a -> Splice n
runChildrenWithNodes = runChildrenWith' (X.renderHtmlFragment X.UTF8)
