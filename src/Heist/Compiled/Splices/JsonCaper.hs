...
--
bindJson :: (ToJSON a, Monad m) => a -> HeistT n m ()
...
------------------------------------------------------------------------------
explodeTag :: (Monad m) => JsonMonad n m [Node]
explodeTag = ask >>= go
  where
    --------------------------------------------------------------------------
    go Null       = goText ""
    ...
    --------------------------------------------------------------------------
    goArray :: (Monad m) => V.Vector Value -> JsonMonad n m [Node]
    goArray a = do
        dl <- V.foldM f id a
        return $! dl []
      where
    ...
    varAttrTag :: (Monad m) => Value -> (JsonMonad n m [Node]) -> HeistT n m
    varAttrTag v m = do
        node <- getParamNode
    ...
    --------------------------------------------------------------------------
    genericBindings :: Monad m => JsonMonad n m [(Text, HeistT n m)]
