...
module Heist.Compiled.Splices.Json (
  bindJson
) where
...
bindJson = runNodeList <=< runReaderT explodeTag . toJSON
...
------------------------------------------------------------------------------
valueTag :: Monad n => JsonMonad n IO [Node]
valueTag = ask >>= go
  where
    go Null       = txt ""
...
------------------------------------------------------------------------------
explodeTag :: (Monad m) => JsonMonad m IO [Node]
explodeTag = ask >>= go
  where
    --------------------------------------------------------------------------
    go Null       = goText ""
    ...
    --------------------------------------------------------------------------
    goText t = lift $ promiseChildrenWith
        [ ("value"   , return [TextNode t])
        , ("snippet" , asHtml t           )
        ]

    --------------------------------------------------------------------------
    goArray :: (Monad n) => V.Vector Value -> JsonMonad n IO [Node]
    goArray a = do
        dl <- V.foldM f id a
        return $! dl []
      where
    ...
    varAttrTag :: (Monad m) => Value -> (JsonMonad m IO [Node]) -> Splice m
    varAttrTag v m = do
        node <- getParamNode
    ...
    --------------------------------------------------------------------------
    genericBindings :: Monad n => JsonMonad n IO [(Text, Splice n)]
    ...
    goObject obj = do
        ...
        lift $ promiseChildrenWith bindings
