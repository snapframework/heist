...
document transformations happen every time the template is requested.  Caper
is like a compiled version of Heist.  Most of your splice processing is done
up front at load time.  Dynamic information can still be rendered at runtime,
but it is faster than fully interpreting the template like Heist does.
...
Splice proccessing involves traversing the DOM, which is inefficient.  Caper
is designed so that all the DOM traversals happen once at load time in the IO
monad.  This is the "compile" phase.  The type signature for Caper splices is
this.

< type CaperSplice n = HeistT n IO (DList (Chunk n))

We see that where Heist splices ran in the m monad, Caper splices run in the
...
But the key point of the Caper splice type signature is the return value.
Caper splices return a DList of Chunks.  DList is a list that supports
efficient insertion to both the front and back of the list.  The Chunk type
defined like this.
...
First this function loads the templates with the above Caper splice.  You have
to specify all the Caper splices in the call to loadTemplates because
loadTemplates takes care of compiling all the templates with caper.  If you
were able to bind Caper splices later, then all the templates would have to be
